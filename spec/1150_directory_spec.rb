...
module Nugem
  class TemplateTest
    RSpec.describe 'Directory entry processing' do
      def cleanup(dest_path_fq)
        dirs = Dir.glob('*', base: dest_path_fq)
        if dirs.empty?
          puts "Temporary directory #{dest_path_fq} is empty."
        else
          puts "Contents of #{dest_path_fq} were, before deletion:"
          puts "  #{dirs.join("\n  ")}"
        end
        FileUtils.rm_rf dest_path_fq
      end

      context 'when handling a regular file' do
      ...
      context 'when handling a template' do
        dest_base = Dir.mktmpdir('nugem_spec_') # Cannot use let vars in after clause
        nugem = Nugem.new({
                            force:    true,
                            gem_name: 'my_gem',
        ...
        it 'copies to a file with a new name, set mode, and render contents' do
          nugem.directory_entry source_path_fq, dest_path_fq, path_fragment.end_with?('.tt')
          expect(File.exist?(dest_path_fq)).to be true
          # TODO: check mode
